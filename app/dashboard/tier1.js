            ...
            {products
              ...
              .map((p) => (
                <li key={p.id} className="border-b border-zinc-700 pb-1 flex justify-between items-center">
                  ...
                  {p.voltage && <span>Voltage: {p.voltage}</span>}
                  {p.max_pressure && <span>Pressure Range: {p.max_pressure}</span>}
                  {p.medium_temperature && <span>Temperature: {p.medium_temperature}</span>}
